...
function genContentTweakerScripts() {
    const stoneVariants = _
        ...
        .join(',')
        .value();
    fs.writeFileSync(`../scripts/custom/GregtechOreMaterials.zs`, _.flattenDeep([
        '#priorty 9999',
        ...
        '}',
        '',
        _.map(worldgen.additionalOres, (o) => {
            const name = `ore${o.name}`;
            return [
                '/*',
                ` * ${o.name}`,
                ' */',
                `var ${name} = MaterialSystem`,
                '    .getMaterialBuilder()',
                `    .setName("${o.name}")`,
                `    .setColor(${o.color})`,
                '    .build()',
                ...
                '    .getData();',
                '',
                `${name}.addDataValue("variants", "${o.variants.map((v) => v.block).join(',')}");`,
                `${name}.addDataValue("hardness", "${o.hardness || 1}");`,
                `${name}.addDataValue("resistance", "${o.resistance || 1}");`,
                `${name}.addDataValue("harvestTool", "${o.tool || 'pickaxe'}");`,
                `${name}.addDataValue("harvestLevel", "${o.level || 1}");`,
                ''
            ]
        })
    ]).join('\n'));
